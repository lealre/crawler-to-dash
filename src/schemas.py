...
class StageSchema(pa.DataFrameModel):
    ...
    num_bedrooms: Series[str] = pa.Field(isin=values_accepted, nullable=True)
    ...
    is_available: Series[bool] = pa.Field(nullable=False)
    site: Series[str] = pa.Field(isin=['sapo'], nullable=True)
    link_id: Series[str] = pa.Field(nullable=False, unique=True)
    link: Series[str] = pa.Field(nullable=False)
